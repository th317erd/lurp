...
function mutateScript(scriptSrc, namespaceHash) {
  let result = `let _LURP=await import("mythix-ui-core");${scriptSrc}`
...
class Require extends _Component {
  async mounted() {
    ...
    try {
      ...
      console.error(`joy-require: Failed to load specified resource: ${src}`, error);
    }
  }
...
customElements.define('joy-require', Require);
