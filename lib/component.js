...
function ensureDocumentStyles(ownerDocument, componentName, template) {
  ...
  for (let templateChild of templateChildren) {
    ...
    if (!ownerDocument.head.querySelector(`#${styleID}`)) {
      ...
      styleNode.setAttribute('data-for', this.elementName);
...
export class Component extends HTMLElement {
  ...
  $(...args) {
    ...
    if (options.shadow !== false && element === this) {
      shadowNodes = Array.from(
        QueryEngine.from.call(this, this.shadow, selector, callback).values(),
      );
    }

    let query = QueryEngine.from.call(this, element, selector, callback);
    if (shadowNodes)
      query = query.addToQuery(shadowNodes);
  ...
  createDynamicProperty(name, _value) {
