...
export class ElementDefinition {
  static [Symbol.hasInstance](instance) {
    try {
      return (instance && instance[Utils.MYTHIX_TYPE] === ELEMENT_DEFINITION_TYPE);
    } catch (e) {
      return false;
    }
  }
...
export function processElements(_node, _options) {
  ...
  const isValidNodeType = (item) => {
    ...
    if (item[Utils.MYTHIX_TYPE] === QUERY_ENGINE_TYPE)
      return true;
    ...
    if (!isTemplateEngineDisabled) {
      ...
      if ((Array.isArray(result) && result.some(isValidNodeType)) || isValidNodeType(result)) {
        ...
        for (let item of result) {
          ...
          } else if (item[Utils.MYTHIX_TYPE] === ELEMENT_DEFINITION_TYPE) {
