...
export const UNFINISHED_DEFINITION = Symbol.for('/joy/elementDefinition/constants/unfinished');
...
export class ElementDefinition {
  ...
  build(document, context) {
    ...
    if (this.tagName === '#text')
      return document.createTextNode(this.formatTerm(context, attributes.value || ''));
    if (namespaceURI)
    ...
    for (let i = 0, il = attributeNames.length; i < il; i++) {
      ...
      element.setAttribute(this.toDOMAttributeName(attributeName), attributeValue);
    }
...
export function build(tagName, defaultAttributes, scope) {
  ...
  const finalizer = (..._children) => {
    let children = _children.map((value) => {
      if (value === false || value == null || Object.is(value, NaN))
        return null;
...
export const Term = (() => {
  const method = (value) => new ElementDefinition('#text', { value });

  Object.defineProperty(method, UNFINISHED_DEFINITION, {
    writable:     false,
    enumerable:   false,
    configurable: false,
    value:        true,
