...
export function createDynamicPropertyFetcher(context, _functionBody, _contextCallArgs) {
  ...
  return (new Function('getDynamicPropertyForPath', 'specialClosest', contextCallArgs, functionBody)).bind(this, getDynamicPropertyForPath, specialClosest);
}
...
export function formatTerm(context, _text) {
  ...
  let result = text.replace(/(?:^\{\{|([^\\])\{\{)([^}]+?)\}{2,}/g, (m, start, macro) => {
    ...
    if (node && value instanceof DynamicProperty) {
      value.registerForUpdate(node, () => {
        // Prevent multiple updates to the
        // same node in the same JS frame
        if (cachedResult != null)
          return cachedResult;

        cachedResult = formatTerm(context, text);
        return cachedResult;
      });
    }
...
const SKIP_PROTOTYPES       = [ HTMLElement, Node, Element, Object, Array ];
